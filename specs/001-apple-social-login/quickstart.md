...
- 실제 Git It 서버 endpoint는 이 기능의 범위 밖이다. 자동화 검증은 명시적으로 주입한
  `MockAuthenticationRemote`를 사용한다.
...
1. 빈 Mock 세션 저장소와 새 Apple 사용자 식별자로 앱 root를 시작한다.
2. Apple 로그인 제어를 한 번 탭하고, 교환이 끝나기 전에 다시 탭한다.
3. Mock이 유효한 Apple 결과와 새 Git It 사용자·세션을 반환하게 한다.

기대 결과: 인증 시도는 하나만 시작되고, access/refresh token은 Keychain Adapter로 한 번
저장되며 인증된 첫 화면으로 이동한다. Feature/App 상태와 테스트 실패 출력에는 토큰이 없다.
...
1. Apple authorization client가 `canceled`를 반환하게 한다.
2. 별도 실행에서 state 불일치, token/code 누락, 네트워크 오류를 각각 반환하게 한다.
3. 오류 뒤 다시 정상 인증을 실행한다.

기대 결과: 취소는 경고 없이 로그인 화면에 남고 저장을 만들지 않는다. 다른 오류는 이해 가능한
재시도 안내를 보이며 보호된 화면을 열지 않는다. 마지막 정상 시도는 이전 오류에 영향받지 않고
완료된다.
...
3. credential state 조회 오류, refresh 네트워크 오류, refresh 거부를 각각 실행한다.

기대 결과: 유효 세션과 refresh 성공은 Apple UI 없이 인증된 첫 화면으로 이동한다. 일시 오류는
토큰을 유지하고 재시도를 제공하며, 거부는 access/refresh token과 Apple 사용자 식별자를 모두
지운 후 로그인 화면으로 이동한다.

### 5. Apple 연결 철회와 로그아웃 경쟁

1. 저장 세션의 credential state를 `.revoked` 또는 `.notFound`로 반환하거나 revoked 알림을
   보낸다.
2. 별도 실행에서 refresh가 진행 중일 때 로그아웃을 탭하고, 그 뒤 refresh 성공 결과를 전달한다.
3. 서버 리프레시 토큰 폐기 요청의 성공과 네트워크 실패를 각각 재현한다.

기대 결과: 철회는 세션 전체를 삭제한다. 로그아웃은 3초 이내 로그인 화면으로 전환하고 서버
폐기 실패에도 되돌아가지 않는다. 늦은 refresh 성공은 Keychain이나 인증 화면을 되살리지 않는다.
...
4. 실제 서버 Adapter가 아직 없으면 이 단계는 Apple authorization client가 계약대로 결과를
   넘기는지까지만 확인하고, 계정 교환·세션 성공은 Mock 자동화 시나리오로 검증한다.

기대 결과: 표준 Apple UI가 표시되고, 진행·취소·오류·재시도 제어가 VoiceOver로 식별된다.
실제 서버 연결 없이 성공한 Git It 세션을 표시해서는 안 된다.
