...
**분리 불가 근거**: `LocalNotificationClient`의 연산 시그니처와 중첩 enum 위치가 바뀌므로
유일한 소비자인 Composition의 스케줄러·게이트웨이 어댑터가 같은 변경에서 함께 바뀌지 않으면
컴파일되지 않는다.
...
- [ ] T009 [S1] `sources/Projects/Infrastructure/PushMessaging/Local/Models/LocalNotificationRequest.swift`를
...
- [ ] T010 [S1] `sources/Projects/Infrastructure/PushMessaging/Clients/LocalNotificationClient.swift`를
      ...
      `present(_ request: LocalNotificationRequest)`로 바꾼다. 최상위
      `LocalNotificationAuthorizationOutcome`를 프로토콜 안 `AuthorizationOutcome`으로 중첩해
      파일 하나에 파일 밖 참조 타입이 하나만 남게 한다(FR-003).
- [ ] T011 [S1] `sources/Projects/Infrastructure/PushMessaging/Clients/UserNotificationCenterLocalClient.swift`를
...
- [ ] T012 [P] [S1] `sources/Projects/Infrastructure/PushMessaging/AppDelegates/FirebaseMessagingAppDelegate.swift`를
      `sources/Projects/Infrastructure/PushMessaging/Remote/AppDelegates/FirebaseMessagingAppDelegate.swift`로
      옮긴다.
- [ ] T013 [P] [S1] `sources/Projects/Infrastructure/PushMessaging/Clients/PushMessagingClient.swift`를
      `sources/Projects/Infrastructure/PushMessaging/Remote/Clients/PushMessagingClient.swift`로 옮긴다.
- [ ] T014 [P] [S1] `sources/Projects/Infrastructure/PushMessaging/Clients/FirebaseMessagingPushClient.swift`를
      `sources/Projects/Infrastructure/PushMessaging/Remote/Clients/FirebaseMessagingPushClient.swift`로 옮긴다.
- [ ] T015 [S1] `sources/Projects/Infrastructure/PushMessaging/Models/PushNotificationHandlers.swift`를
...
- [ ] T016 [P] [S1] `sources/Projects/Infrastructure/PushMessaging/Models/RemoteNotificationPayload.swift`를
...
- [ ] T017 [S1] `sources/Projects/Composition/Adapter/Factories/GenerationCompletionReminderCoordinator.swift`에서
      알림 식별자 `"generation-completed-<projectID>"`와 제목·본문 문구를 생성해
      `LocalNotificationRequest`로 `present(_:)`에 전달하도록 바꾼다. 형식과 내용은 현재와
      동일하게 유지한다(FR-018).
- [ ] T018 [S1] `sources/Projects/Composition/Adapter/Adapters/NotificationAuthorizationGatewayAdapter.swift`가
      중첩된 `LocalNotificationClient.AuthorizationOutcome`을 참조하도록 바꾼다.
- [ ] T019 [S1] `sources/Projects/Composition/Adapter/Assemblies/AppComposition.swift`와
      `sources/Projects/Composition/Adapter/Factories/PushNotificationAppDelegate.swift`가
      `PushNotificationCallbacks`를 참조하도록 바꾼다.

### 단위 검증

- [ ] T020 [no-write] [S1] `build`를 실행하고
      `grep -rniE "세트|학습|generation-completed|생성 완료" sources/Projects/Infrastructure/PushMessaging --include="*.swift"`가
      0줄임을 확인한다(SC-001).

**진행 점검**: T009~T020의 변경 파일과 검증 결과를 보고하고 U2로 연속 진행한다.
...
- [ ] T021 [S2] `sources/Projects/Domain/LearningProject/Models/LearningProject/LearningProjectGenerationOutcome.swift`를
...
- [ ] T022 [S2] `sources/Projects/Domain/LearningProject/Contracts/LearningProjectGenerationOutcomeRepository.swift`를
...
- [ ] T023 [P] [S2] `sources/Projects/Data/LearningProject/DTOs/ProjectGenerationOutcomeDTO.swift`를
...
- [ ] T024 [S2] `sources/Projects/Data/LearningProject/Contracts/ProjectGenerationOutcomeRemote.swift`를
...
- [ ] T025 [S2] `sources/Projects/Data/LearningProject/Remotes/PushProjectGenerationOutcomeRemote.swift`를
...
- [ ] T026 [P] [S2] `sources/Projects/Data/Tests/LearningProject/DTOs/ProjectGenerationOutcomeDTOTests.swift`를
      `sources/Projects/Data/Tests/LearningProject/DTOs/GenerationOutcomeDTOTests.swift`로 옮기고
      새 타입 이름을 참조하도록 바꾼다.
- [ ] T027 [P] [S2] `sources/Projects/Data/Tests/LearningProject/Remotes/PushProjectGenerationOutcomeRemoteTests.swift`를
      `sources/Projects/Data/Tests/LearningProject/Remotes/PushGenerationOutcomeStreamTests.swift`로
      옮기고 새 타입 이름을 참조하도록 바꾼다.

### 구현 — Composition

- [ ] T028 [S2] `sources/Projects/Composition/Adapter/Adapters/LearningProjectGenerationOutcomeRepositoryAdapter.swift`를
...
- [ ] T029 [S2] `sources/Projects/Composition/Adapter/Assemblies/LearningProjectAssembly.swift`가
      새 타입 이름을 참조하도록 바꾼다.
- [ ] T030 [S2] `sources/Projects/Composition/Adapter/Factories/GenerationCompletionReminderCoordinator.swift`가
      `GenerationOutcome`을 참조하도록 바꾼다.
- [ ] T031 [P] [S2] `sources/Projects/Composition/Tests/Adapter/Adapters/LearningProjectGenerationOutcomeRepositoryAdapterTests.swift`를
...
- [ ] T032 [P] [S2] `sources/Projects/Composition/Tests/Adapter/Assemblies/LearningProjectAssemblyTests.swift`와
      `sources/Projects/Composition/Tests/Adapter/Factories/GenerationCompletionReminderCoordinatorTests.swift`가
      새 타입 이름을 참조하도록 바꾼다.

### 구현 — Feature

- [ ] T033 [S2] `sources/Projects/Feature/Home/Reducers/HomeFeature.swift`와
      `sources/Projects/Feature/Home/Previews/HomePreviewSupport/HomePreviewSupport.swift`가
      `GenerationOutcome`을 참조하도록 바꾼다.
- [ ] T034 [S2] `sources/Projects/Feature/ProjectRegistration/Reducers/ProjectRegistrationFeature.swift`와
...
- [ ] T035 [P] [S2] `sources/Projects/Feature/Tests/Home/Reducers/HomeFeatureGenerationOutcomeTests.swift`,
...
- [ ] T036 [S2] `sources/Projects/App/GitIt/Screens/AppRootView.swift`와
      `sources/Projects/App/Tests/GitIt/TestDoubles/LearningProjectOutcomesUseCaseMock.swift`가
      `GenerationOutcome`을 참조하도록 바꾼다.

### 단위 검증

- [ ] T037 [no-write] [S2] `build`·`compile`·`test`를 실행하고
...
**진행 점검**: T021~T037의 변경 파일과 검증 결과를 보고하고 U3로 연속 진행한다.
...
- [ ] T038 [S2] `sources/Projects/Domain/LearningProject/UseCases/LearningProjectOutcomes/LearningProjectOutcomesUseCase.swift`를
...
- [ ] T039 [S2] `sources/Projects/Domain/LearningProject/UseCases/LearningProjectOutcomes/LearningProjectOutcomes.swift`를
...
- [ ] T040 [P] [S2] `sources/Projects/Domain/LearningProject/Models/NotificationAuthorizationOutcome.swift`를
...
- [ ] T041 [S2] `sources/Projects/Domain/LearningProject/UseCases/RequestGenerationReminder/RequestGenerationReminderUseCase.swift`에서
      `isAuthorized()`를 제거해 계약이 이름이 설명하는 책임만 갖게 한다(FR-007).
- [ ] T042 [S2] `sources/Projects/Domain/LearningProject/UseCases/RequestGenerationReminder/RequestGenerationReminder.swift`에서
      `isAuthorized()` 구현을 제거한다.
- [ ] T043 [S2] `sources/Projects/Domain/LearningProject/UseCases/NotificationAuthorizationStatus/NotificationAuthorizationStatusUseCase.swift`를
      생성하고 `func callAsFunction() async -> Bool`을 선언한다.
- [ ] T044 [S2] `sources/Projects/Domain/LearningProject/UseCases/NotificationAuthorizationStatus/NotificationAuthorizationStatus.swift`를
      생성하고 `NotificationAuthorizationGateway`에 위임하는 구현을 작성한다.
- [ ] T045 [P] [S2] `sources/Projects/Domain/Tests/LearningProject/UseCases/LearningProjectOutcomesTests.swift`를
      `sources/Projects/Domain/Tests/LearningProject/UseCases/ObserveGenerationOutcomesTests.swift`로
      옮기고 새 이름을 참조하도록 바꾼다.
- [ ] T046 [S2] `sources/Projects/Domain/Tests/LearningProject/UseCases/RequestGenerationReminderTests.swift`에서
...
- [ ] T047 [S2] `sources/Projects/Composition/Adapter/Assemblies/LearningProjectAssembly.swift`의
      `learningProjectOutcomes` 프로퍼티를 `observeGenerationOutcomes`로 바꾸고 새 타입을 조립한다.
- [ ] T048 [S2] `sources/Projects/Composition/Adapter/Assemblies/AppComposition.swift`의
...
- [ ] T049 [P] [S2] `sources/Projects/Composition/Adapter/Factories/GenerationCompletionReminderCoordinator.swift`와
      `sources/Projects/Composition/Tests/Adapter/**`의 관련 파일이 새 이름을 참조하도록 바꾼다.

### 구현 — Feature

- [ ] T050 [S2] `sources/Projects/Feature/Home/Reducers/HomeFeature.swift`,
...
- [ ] T051 [S2] `sources/Projects/Feature/ProjectRegistration/Reducers/ProjectRegistrationFeature.swift`가
...
- [ ] T052 [P] [S2] `sources/Projects/Feature/ProjectRegistration/Previews/ProjectRegistrationScreenPreviews.swift`,
...
- [ ] T053 [S2] `sources/Projects/App/GitIt/Reducers/AppRootFeature.swift`,
      `sources/Projects/App/GitIt/Screens/AppRootView.swift`,
...
- [ ] T054 [no-write] [S2] `build`·`compile`·`test`를 실행하고
...
**진행 점검**: T038~T054의 변경 파일과 검증 결과를 보고하고 U4로 연속 진행한다.
...
- [ ] T055 [S3] `sources/Projects/Composition/Adapter/Factories/GenerationCompletionReminderCoordinator.swift`를
...
- [ ] T056 [S3] `sources/Projects/Composition/Adapter/Assemblies/GenerationReminderScheduler.swift`가
      `GenerationReminderRegistry`를 직접 구현하도록 바꾸고, `start(...)`가
      `Task<Void, Never>`를 반환하게 하며 `waitUntilObservationFinished()`를 제거한다(FR-010).
- [ ] T057 [S3] `sources/Projects/Composition/Adapter/Adapters/GenerationReminderRegistryAdapter.swift`를
      삭제한다. 변환 없는 순수 위임이므로 T056이 대체한다(FR-013).
- [ ] T058 [S3] `sources/Projects/Composition/Adapter/AppDelegates/PushNotificationAppDelegate.swift`를
...
- [ ] T059 [S3] `sources/Projects/Composition/Adapter/Factories/PushNotificationAppDelegate.swift`
      (typealias)를 삭제한다.
- [ ] T060 [S3] `sources/Projects/Composition/Adapter/Assemblies/AppComposition.swift`에서
...
- [ ] T061 [P] [S3] `sources/Projects/Composition/Tests/Adapter/Factories/GenerationCompletionReminderCoordinatorTests.swift`를
...
- [ ] T062 [S3] `sources/Projects/Composition/Tests/Adapter/Assemblies/AppCompositionPublicSurfaceTests.swift`에
      `live(...)` 호출만으로 부작용이 발생하지 않음을 검증하는 테스트를 추가한다(SC-004).

### 구현 — App

- [ ] T063 [S3] `sources/Projects/App/GitIt/GitItApp.swift`가 조립 후 `composition.start()`를
      호출하고, `@UIApplicationDelegateAdaptor`에 Composition 소유
      `PushNotificationAppDelegate`를 사용하도록 바꾼다.

### 구현 — 공용 문서

- [ ] T064 [S3] `docs/conventions/file-vocabulary.md` §3의 형태 어휘 표에서
...
- [ ] T065 [no-write] [S3] `make tuist` 후 `build`·`test`를 실행하고 quickstart 시나리오 3의
      세 grep이 0줄임을 확인한다. `make tuist` 실행 전후 `git status`를 비교한다.

**진행 점검**: T055~T065의 변경 파일과 검증 결과를 보고하고 U5로 연속 진행한다.
...
- [ ] T066 [S4] `sources/Projects/UI/Component/Controls/TextField.swift`를 삭제한다.
      저장소 전체에서 production 참조가 0이며 남은 참조는 자기 파일의 `#Preview` 3건과
      아래 테스트뿐이다([research.md](./research.md) R-007).
- [ ] T067 [S4] `sources/Projects/UI/Tests/Component/Unit/Controls/TextFieldTests.swift`를 삭제한다.

### 단위 검증

- [ ] T068 [no-write] [S4] `build`·`test`를 실행하고 두 경로가 존재하지 않음을 확인한다.

**진행 점검**: T066~T068의 변경 파일과 검증 결과를 보고하고 U6으로 연속 진행한다.
...
- [ ] T069 [S4] `sources/Projects/Feature/ProjectRegistration/Reducers/ProjectRegistrationFeature.swift`에서
      `Delegate.notificationOptionSelected(accepted:)` 케이스와 이를 발행하는
      `finishWaiting`의 분기를 제거한다. 이 출력은 유일한 소비자가 무시한다(FR-013).
- [ ] T070 [S4] 같은 파일에서 `CancelID.submission`을 제거한다. 저장소에 이 id를 취소하는
      코드가 없고 `cancelInFlight`도 없어 사용되지 않는다. 중복 제출은 기존
      `state.submission != .committing` guard가 계속 막는다.
- [ ] T071 [S4] `sources/Projects/App/GitIt/Reducers/AppRootFeature.swift`에서
      `.projectRegistration(.presented(.delegate(.notificationOptionSelected(_))))` 빈 분기를 제거한다.
- [ ] T072 [P] [S4] `sources/Projects/Feature/ProjectRegistration/Screens/NotificationOptionSheet.swift`에서
      미사용 상수 `Constant.bellIconSize`를 제거한다.
- [ ] T073 [S4] `sources/Projects/Feature/Home/Reducers/HomeFeature.swift`에서 단방향 2케이스
...
- [ ] T074 [S4] `sources/Projects/Feature/Tests/ProjectRegistration/Reducers/ProjectRegistrationFeatureTests.swift`에서
      제거된 delegate를 검증하는 `receive` 구문 6곳을 제거한다.
- [ ] T075 [P] [S4] `sources/Projects/Feature/Tests/Home/Reducers/HomeFeatureGenerationOutcomeTests.swift`가
      새 `Bool` 상태를 참조하도록 갱신한다.

### 단위 검증

- [ ] T076 [no-write] [S4] `build`·`test`를 실행하고
...
**진행 점검**: T069~T076의 변경 파일과 검증 결과를 보고하고 U7로 연속 진행한다.
...
- [ ] T077 [S4] `알림 수락은 notificationOptionSelected accepted true를 출력한 뒤 waitAtHomeTapped
      동작을 이어간다` 테스트를 삭제한다. 뒤따르는 `권한이 허용되면 …` 테스트와 send/receive
      시퀀스가 동일하고 후자가 spy 검증까지 포함해 완전히 포함한다.
- [ ] T078 [S4] `권한이 허용되면 …`, `권한을 방금 거부해도 …`, `권한이 이미 거부된 상태면 …`
...
- [ ] T079 [S4] `알림 옵션 선택은 상태 전이 자체에 영향을 주지 않는다` 테스트를 삭제한다.
      검증 대상 필드(`repositoryURLInput`, `quizLevel`)를 한 번도 설정하지 않아 항상 통과하며
      `exhaustivity = .off`로 실제 상태 변화도 확인하지 않는다(FR-015).

### 단위 검증

- [ ] T080 [no-write] [S4] `test`를 실행하고 통합 전후 검증 입력 조합 수를 비교해 줄지
      않았음을 확인한다.

**진행 점검**: T077~T080의 변경 파일과 검증 결과를 보고하고 U8로 연속 진행한다.
...
- [ ] T081 [S5] `sources/Tuist/ProjectDescriptionHelpers/Projects/InfrastructureModuleName.swift`에
      ...
      포함한다.
- [ ] T082 [S5] `sources/Tuist/ProjectDescriptionHelpers/ProjectName.swift`의 Infrastructure
      프로젝트 `testTargets` 배열에 `InfrastructurePushMessagingTests`를 추가한다.

### 구현 — Infrastructure 테스트

- [ ] T083 [P] [S5] `sources/Projects/Infrastructure/Tests/PushMessaging/Remote/Models/RemoteNotificationPayloadTests.swift`를
      생성하고 `[AnyHashable: Any]` → `[String: String]` 변환에서 비문자열 키가 제외되는지
      검증한다.
- [ ] T084 [P] [S5] `sources/Projects/Infrastructure/Tests/PushMessaging/Local/Models/LocalNotificationRequestTests.swift`를
      생성하고 생성한 `identifier`·`title`·`body`가 그대로 읽히는지 검증한다.

### 구현 — Composition 테스트

- [ ] T085 [S5] `sources/Projects/Composition/Tests/Adapter/Adapters/NotificationAuthorizationGatewayAdapterTests.swift`를
      생성하고 Infrastructure `AuthorizationOutcome`의 세 케이스가 각각 대응하는 Domain
      `NotificationAuthorizationOutcome`으로 변환되는지 검증한다. 현재 이 변환에는 테스트가
      없어 세 분기를 뒤바꿔도 실패하지 않는다(FR-017).

### 구현 — Feature 테스트

- [ ] T086 [S5] `sources/Projects/Feature/Tests/Home/Reducers/HomeFeatureGenerationOutcomeTests.swift`에
...
- [ ] T087 [no-write] [S5] `make tuist` 후 `build`·`compile`·`test`를 실행하고, 실행 기록에
      `InfrastructurePushMessagingTests`가 나타나며 실패가 없음을 확인한다. `make tuist` 실행
      전후 `git status`를 비교해 추적 파일 변경이 생기지 않았는지 확인한다.

**진행 점검**: T081~T087의 변경 파일과 검증 결과를 보고하고 U9로 연속 진행한다.
...
- [ ] T088 [no-write] `make tuist` 후 `build`·`compile`·`test`를 순차 실행한다. `make tuist`
      실행 전후 `git status`를 비교해 추적 파일 변경이 생기면 완료로 처리하지 않고 변경 경로와
      영향을 보고한다.
- [ ] T089 [no-write] `/tmp/gitit-baseline-tests.log`(T003)와 현재 테스트 결과를 비교해 변경 전
      통과하던 테스트가 모두 통과하고 새로 실패하는 테스트가 0건임을 확인한다(SC-003).
      이름이 바뀐 테스트는 대응 관계를 보고에 기록한다.
- [ ] T090 [no-write] [quickstart.md](./quickstart.md)의 시나리오별 grep 검증 5개를 모두
      실행해 SC-001·SC-002·SC-004·SC-005·SC-006을 확인한다.
- [ ] T091 [no-write] SC-007을 확인한다.
      `sources/Projects/Composition/Adapter/Adapters/NotificationAuthorizationGatewayAdapter.swift`의
      매핑 한 줄을 임시로 뒤바꿔 T085의 테스트가 실패하는지 확인한 뒤 원상복구한다. 이 임시
      변경은 커밋하지 않으며 복구 후 `git status`가 깨끗한지 확인한다.
- [ ] T092 [no-write] `./tools/script-verification/bin/run.sh`를 실행해 정적 검사와 회귀
      테스트를 확인한다(SC-009).
- [ ] T093 마지막 커밋 단위를 stage하기 전에 포맷 훅을 실행하고, 허용된 포맷 결과를 같은
...
| U1 | T012, T013, T014, T016 | 서로 다른 파일의 단순 이동 |
| U2 | T023 / T026·T027 / T031·T032 / T035 | Data·Composition·Feature 테스트가 서로 독립 |
| U3 | T040, T045 / T049 / T052 | 모델 이동과 테스트 갱신이 서로 독립 |
| U4 | T061 | 다른 Composition 소스와 파일이 겹치지 않음 |
| U6 | T072, T075 | Sheet 상수와 Home 테스트가 서로 독립 |
| U8 | T083, T084 | 서로 다른 신규 테스트 파일 |
...
| `make tuist`가 추적 파일 diff를 바꾼 경우 | T008, T065, T087, T088 |
...
| --- | --- | --- | --- |
| S1 Infrastructure 경계 | P1 | T009~T020 | Infrastructure 소스에 서비스 용어·표시 문구 0건, 파일당 최상위 타입 1개 |
| S2 어휘 통일 | P2 | T021~T054 | 옛 어휘 grep 0줄, 최장 이름 40자 이하, 기존 테스트 전부 통과 |
| S3 조립/실행 분리 | P3 | T055~T065 | `live(...)` 부작용 0건, Composition 공개 API에 Infrastructure 타입 0건 |
| S4 죽은 코드·중복 테스트 | P4 | T066~T080 | 죽은 선언 grep 0줄, 검증 입력 조합 수 유지 |
| S5 검증 공백 | P5 | T081~T087 | 새 테스트 target이 실행에 포함되어 통과, 매핑 분기 뒤바꾸면 실패 |

## 최소 가치 범위

**U0 + U1 (T001~T020)** 만으로도 유일하게 문서화된 패키지 규칙 위반이 해소되어 독립적인
