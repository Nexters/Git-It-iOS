...
PR이 Markdown, 문서 이미지, `specs/`, `sources/docs/` 등 문서 파일만 변경한 경우, macOS runner에서 Swift lint, 앱 빌드, 단위 테스트, UI 테스트를 실행하지 않고 변경 분류와 기본 문서 검사만 수행한 뒤 `CI / gate`를 성공으로 완료한다. 이를 통해 문서 기여의 CI 대기시간을 p50 2분 이하로 단축한다.
