...
| `implementationLocation` | 구현 예정 위치 | 참조 화면에 필수, 나머지는 후속 기능에서 추가 |
| `plannedTarget` | `구현 대상` · `후속 기능` · `제외` | 계획 시 책임만 표현하고 동적인 완료 상태는 저장하지 않음 |
| `verificationStates` | 검증 대상 상태 목록 | 참조 화면의 디자인 상태 5개·운영 상태 2개 |

**규칙**: 이름이 같은 프레임은 내용을 대조하기 전에는 별개 화면으로 집계하지 않고
`내용 미대조`로 표시한다. 자식 구성이 같고 특정 구성 요소의 변형만 다르면 한 화면의 상태
변형이다(R-08). 이 기능은 참조 화면만 상태·구현 위치·계획 대상을 모두 확정하며 나머지
화면의 상세 대조는 담당 후속 기능이 수행한다. 현재 완료 상태는 이 엔터티에 저장하지 않고
`tasks.md`의 완료 표시와 실제 빌드·테스트 결과로 판정한다.
...
| `reviewBoundary` | 화면 단위 검수 경계 | 포함 화면마다 담당 범위를 식별 가능해야 함 |
| `tokenOwner` | 전역 토큰 소유자 | `006 UI 기반` 고정, 후속 그룹의 재정의 금지 |

**운영 규칙**: 후속 기능의 패키지 순서는 이 모델에 고정값으로 저장하지 않고 실행 시점의
Constitution 원칙 7을 참조한다. 공용 컴포넌트는 단일 구현 기능이 소유하고 전역
DesignSystem 토큰은 006 UI 기반이 소유한다.
