...
struct SelectionCardListTests {
    ...
    func `onSelect callback은 선택한 item id를 그대로 전달한다`() {
        ...
        _ = SelectionCardList(items: items, onSelect: onSelect)
    ...
    func `각 item은 자신에게 지정된 illust를 유지한다`() {
        let items: [SelectionCardList.Item] = [
            .init(id: "basic", title: "기초", illust: .knowledgeBasic),
            .init(id: "advanced", title: "심화", illust: .knowledgeAdvanced),
        ]

        #expect(items[0].illust == .knowledgeBasic)
        #expect(items[1].illust == .knowledgeAdvanced)
    }

    @Test
    func `직접 입력으로 생성하고 onSelect 기본값은 생략할 수 있다`() {
        let items: [SelectionCardList.Item] = [
            .init(id: "concept", title: "기술 개념은 알아요", supportingText: "흐름 중심 학습")
        ]

        _ = SelectionCardList(items: items)
