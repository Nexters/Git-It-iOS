...
struct LocalPolicyConsentStoreTests {
    ...
    func `logout과 무관하게 유지되고 앱 데이터 삭제를 시뮬레이션하면 부재로 돌아간다`() async {
        ...
        // logout에 해당하는 별도 동작이 없어도 같은 UserDefaults suite를 쓰는 새 인스턴스는 기록을 그대로 조회한다.
        let afterLogout = LocalPolicyConsentStore(store: UserDefaultsStore(namespace: "legal-consent", userDefaults: userDefaults))
        #expect(await afterLogout.records().isEmpty == false)

        userDefaults.removePersistentDomain(forName: suiteName)

        let afterAppDataDeletion = LocalPolicyConsentStore(store: UserDefaultsStore(namespace: "legal-consent", userDefaults: userDefaults))
    ...
    private func makeStore() -> (LocalPolicyConsentStore, UserDefaults) {
        let suiteName = "policy-consent-\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        return (LocalPolicyConsentStore(store: UserDefaultsStore(namespace: "legal-consent", userDefaults: userDefaults)), userDefaults)
    }
