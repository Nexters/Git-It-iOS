...
struct PushQuizGenerationOutcomeSourceTests {
    ...
    func `ingest 한 번으로 두 구독 스트림이 같은 이벤트를 받는다`() async {
        ...
        let stream1 = await source.outcomes()
        let stream2 = await source.outcomes()
    ...
    func `디코딩 실패 payload는 조용히 폐기된다`() async {
        let source = PushQuizGenerationOutcomeSource()
        let stream = await source.outcomes()

        await source.ingest(rawPayload: ["projectId": "project-1"])
        await source.ingest(rawPayload: ["projectId": "project-2", "status": "completed"])
