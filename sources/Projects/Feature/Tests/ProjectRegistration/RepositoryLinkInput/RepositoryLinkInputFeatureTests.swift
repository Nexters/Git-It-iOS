...
struct RepositoryLinkInputFeatureTests {
    ...
    func `공유 초기값이 있으면 task가 검증을 1회 자동 시작한다`() async {
        ...
        let store = makeRepositoryLinkInputStore(
            ...
            state: RepositoryLinkInputFeature.State(initialRepositoryURL: "https://github.com/owner/repo"),
        )
    ...
    func `자동 검증 실패도 직접 누른 경우와 같은 failed 상태로 전이한다`() async {
        ...
        let store = makeRepositoryLinkInputStore(
            fetchExternalRepository: fetchExternalRepository,
            state: RepositoryLinkInputFeature.State(initialRepositoryURL: "https://example.com/article"),
        )
