...
struct AppleAuthorizationProviderTests {
    ...
    func `명시적으로 시작한 시도만 유효한 credential을 반환한다`() throws {
        let provider = AppleAuthorizationProvider(
            randomValue: { String(repeating: "x", count: $0) },
        ...
        #expect(try provider.complete(credential: credential, state: attempt.state, attemptID: attempt.id) == credential)
    ...
    func `취소와 불일치 또는 만료 및 누락 credential을 격리한다`() throws {
        ...
        let cancelledProvider = AppleAuthorizationProvider(randomValue: { String(repeating: "x", count: $0) }, now: { now })
        ...
        let invalidProvider = AppleAuthorizationProvider(randomValue: { String(repeating: "x", count: $0) }, now: { now })
        ...
        #expect(throws: AppleAuthorizationError.invalidCallback) {
            try invalidProvider.complete(credential: credential, state: "other", attemptID: invalidAttempt.id)
        }
        let missingProvider = AppleAuthorizationProvider(randomValue: { String(repeating: "x", count: $0) }, now: { now })
        ...
        #expect(throws: AppleAuthorizationError.missingCredential) {
            try missingProvider.complete(credential: credential, state: missingAttempt.state, attemptID: missingAttempt.id)
        }
        let expiredProvider = AppleAuthorizationProvider(randomValue: { String(repeating: "x", count: $0) }, now: { now })
