...
**커밋 단위 구현**: `/speckit-implement`는 선택한 한 패키지의 미완료 작업을 실행 시점에
논리적이고 독립적으로 되돌릴 수 있는 커밋 단위로 설계한다. 각 단위는 작업 ID, 정확한 파일,
...
별도 commit으로 확정하고, 구현 중에는 단일 중단 단위의 checkbox diff만 재개한다. 후속 패키지는
직전 결과 보고 뒤 현재 사용자 입력의 명시적 승인으로만 시작한다.
...
**패키지 진행**: 현재 명세가 변경하는 패키지를 식별하고 의존성 위상 순서로 구현 경계를
계획한다. 피의존 패키지를 먼저 두며 근거는 아키텍처 문서의 패키지 의존성 표다. 서로
의존하지 않는 패키지의 상대적 순서는 이 계획이 정하고 근거를 남긴다. 적용되지 않는 패키지는
