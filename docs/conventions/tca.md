...
- `sources/Projects/Feature/**`의 TCA Feature와 화면에 적용합니다.
...
- Router보다 긴 생명주기가 필요한 상태는 Router의 State나 화면 Feature의 State에
  두지 않습니다. §4.1·§6.2가 금지하는 TCA `@Shared`, `Binding`, `inout` 기반 공유
  대신, 정본을 가진 상위 계층(App의 State, 또는 Domain/Infrastructure 정본)이 소유하고
  하위 Feature는 `delegate`로 알리거나 필요할 때 생성자 주입 UseCase로 정본을 다시
  조회합니다. Router가 재생성되어도(예: 상위가 로그아웃 후 Router State를 새로
  만드는 경우) 이 정본은 영향받지 않습니다.
