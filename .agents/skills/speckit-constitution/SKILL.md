...
산출물 언어, 세션 지식 기록 위임, 인자 이스케이프 규칙을 그대로 따른다(기록 대상은
`docs/spec-kit/<feature>/` 아래 두 기록 파일).
...
**Check for extension hooks (before constitution update)**: [공통 확장 훅 프로토콜](../../../.specify/memory/speckit-common-rules.md#확장-훅extension-hooks-프로토콜)을
따르되 훅 키는 `hooks.before_constitution`, 필수 훅의 "Wait for..." 대상 섹션은 "the Outline"이다.
...
**Check for extension hooks (after constitution update)**: apply the [공통 확장 훅 프로토콜](../../../.specify/memory/speckit-common-rules.md#확장-훅extension-hooks-프로토콜)
with hook key `hooks.after_constitution`.
