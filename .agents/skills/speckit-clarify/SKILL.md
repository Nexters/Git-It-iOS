...
**Check for extension hooks (before clarification)**: [공통 확장 훅 프로토콜](../../../.specify/memory/speckit-common-rules.md#확장-훅extension-hooks-프로토콜)을
따르되 훅 키는 `hooks.before_clarify`, 필수 훅의 "Wait for..." 대상 섹션은 "the Outline"이다.
...
If no hooks are registered under `hooks.after_clarify`, skip to the Completion Report. Otherwise
apply the [공통 확장 훅 프로토콜](../../../.specify/memory/speckit-common-rules.md#확장-훅extension-hooks-프로토콜)
with hook key `hooks.after_clarify`. For a mandatory hook you MUST emit `EXECUTE_COMMAND:` and
