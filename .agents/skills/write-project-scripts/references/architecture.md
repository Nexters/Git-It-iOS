...
CI의 production build와 unit compile은 `sources/docs/architecture.md`의 패키지 의존성 그래프를 provider부터 consumer까지 위상 단계로 나눈다. `Domain`·`Data`·`Infrastructure`·`UI`를 같은 기반 단계에서 병렬 실행하고, 완료 후 `Composition`·`Feature`를 병렬 실행한 다음 `App`을 실행한다. 모든 production build 단계가 끝난 뒤 compile 단계를 같은 순서로 시작하고, 모든 compile 단계가 끝난 뒤 test를 시작한다. 각 단계 안에서는 scheme 또는 test target을 최소 실행 단위로 유지한다.
