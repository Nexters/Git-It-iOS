...
CI workflow는 단일 build, compile, test job을 순서대로 실행한다. build는 모든 공유 scheme을 빌드하고, compile은 모든 테스트 가능 scheme을 `build-for-testing`한 뒤 `TestSchemes/*/Build/Products`를 동일 실행의 artifact로 전달한다. test는 이 산출물을 같은 Derived Data 경로에 복원하고 `test-without-building`만 실행한다. scheme 또는 test target 단위 job 분할은 사용하지 않는다.
