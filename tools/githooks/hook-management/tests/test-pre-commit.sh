...
design_rule_runner_relative=$("$source_paths" GIT_IT_DESIGN_RULE_RUNNER)
...
for step in script-tests swift-format design-rules build compile; do
...
: >"$PRE_COMMIT_LOG"
printf '%s\n' '# 주석' '' 'compile' 'build' 'design-rules' 'swift-format' 'script-tests' >"$enabled"
...
: >"$PRE_COMMIT_LOG"
printf '%s\n' 'script-tests' 'swift-format' 'design-rules' 'build' 'compile' >"$enabled"
printf '%s\n' '#!/bin/sh' \
	'printf "compile\n" >> "$PRE_COMMIT_LOG"' \
...
mkdir -p "$stage_repository/$hooks_relative/pre-commit.d" \
	...
	"$(dirname -- "$stage_repository/$design_rule_runner_relative")"
...
printf '%s\n' '#!/bin/sh' \
	'printf "design-rules:%s\\n" "$PWD" >> "$PRE_COMMIT_LOG"' >"$stage_repository/$design_rule_runner_relative"
