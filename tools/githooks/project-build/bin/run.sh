...
project_build_main() (
	...
	project_build_module=$(CDPATH='' cd -- "$project_build_bin/.." && pwd -P)
	project_build_paths="$project_build_root/tools/repository-paths/bin/repository-paths.sh"
	[ -x "$project_build_paths" ] || {
		printf '오류[repository-paths.missing-reader]: %s를 실행할 수 없습니다\n조치: 저장소 공용 경로 판독 명령을 복원하세요\n' \
			"$project_build_paths" >&2
		return 2
	}

	# JSON 기본값과 환경변수 override를 실행용 절대경로로 읽습니다.
	project_build_projects=$("$project_build_paths" --absolute GIT_IT_PROJECTS_ROOT) || return $?
	project_build_workspace=$("$project_build_paths" --absolute GIT_IT_WORKSPACE_PATH) || return $?
